...
from op_tester import __version__, bootstrap
...
from op_tester.plans.types import ExecutionPlan, RunOptions
from op_tester.reporting import JsonReporter, ReportManager, TerminalReporter
...
def _print_version(_: click.Context, __: click.Parameter, value: bool) -> None:
    if not value or click.get_current_context().resilient_parsing:
        return
    click.echo(f"op-tester {__version__}")
...
@click.option(
    ...
    expose_value=False,
    is_eager=True,
    help="Show the op-tester version and exit.",
)
@click.pass_context
def cli(ctx: click.Context, verbose: bool) -> None:
    """Top level CLI group for op-tester."""
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    try:
        cli.main(args=argv, prog_name="op-tester", standalone_mode=True)
...
def _default_report_path() -> str:
    ...
    return str(Path.cwd() / f"op_tester_report_{timestamp}.json")
