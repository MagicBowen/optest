# op-tester Architecture & Extension Guide
...
- Reference implementations are Python callables imported via dotted strings (e.g., `op_tester.operators.reference_impls:gemm_reference`).
- Custom generators or references can be registered via config, CLI overrides, or plugin modules loaded through `OP_TESTER_PLUGINS` env var.
...
## 3. Extending op-tester with Custom GPU/NPU Backends
...
2. **Expose driver to the CLI**
   - Create a Python module (e.g., `my_company.op_tester_plugins.backends`) that registers the driver at import time.
   - Set `OP_TESTER_PLUGINS=my_company.op_tester_plugins.backends` before invoking `op-tester`, or import the module in your own CLI wrapper before calling `bootstrap()`.
      ...
      ascend:
        ...
        outputs:
          - tensor: output0
            ...
            dtype: float16
```
- op-tester writes generator-produced tensors to the configured `input/*.bin` files, invokes your command, and expects the output file(s) to appear under `output/`.
...
- For end users: `pip install op-tester` (or your published name/version).
- They’ll receive the `op-tester` console script plus Python API for embedding (e.g., `from op_tester.core.runner import TestRunner`).

### 4.4 Delivering with Custom Plugins
- Package plugins in a separate distribution (e.g., `company-op-tester-plugins`) that depends on `op-tester`.
...
4. Run `op-tester run --plan my_plan.yaml --backend gpu --chip a100 --report json`.
