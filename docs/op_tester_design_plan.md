# op-tester Design & Implementation Plan

## 1. Goals & Scope
- Deliver `op-tester`, a Python package installable via pip that exposes a CLI for validating AI operators targeting CUDA GPUs or diverse NPUs (chip tag aware).
...
### Functional
1. Discover and run operator tests through CLI (`op-tester run`).
...
- Installable package with console script entry point (`op-tester`).
...
3. **Generator API**: `generate(case: TestCase, rng: Generator) -> (inputs, reference_outputs, aux_metadata)`; base implementations live under `op_tester/generators/defaults`.
...
9. **Plugin Loader**: optional env var (e.g., `OP_TESTER_PLUGINS`) listing modules to import at startup so they can register operators/generators/backends.

## 5. CLI Design
- Console script `op-tester` built on `click` (or Typer). Primary command `op-tester run`.
- Options:
  ...
  - `--report {terminal,json}` default terminal; `--report-path PATH` (default `./op_tester_report_<timestamp>.json` when JSON selected).
...
- Plugin discovery: environment variable `OP_TESTER_PLUGINS="pkg1.module,pkg2.module"`; CLI loads them before building the registry.
...
1. **Phase 0 – Project Skeleton & Packaging**
   ...
   - Establish package directories: `op_tester/cli`, `core`, `registry`, `generators`, `backends`, `reporting`, `plans`, `utils`.
   - Wire console entry point `op-tester` invoking CLI stub.
