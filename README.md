...
A Python-based CLI framework for validating AI operators (CUDA GPU / NPU) with autogenerated inputs and NumPy reference implementations. op-tester streamlines authoring test plans, executing kernels against stub or custom backends, and producing terminal/JSON reports for rapid regression diagnostics.
...
See `docs/op_tester_design_plan.md` for the roadmap and `docs/op_tester_architecture.md` for architectural details, extension guidance, and packaging tips.
...
```bash
pip install op-tester  # replace with your published package name if different
```
...
Run it:
```bash
op-tester run --plan plan.yaml --report json
```
...
`workdir` points at your operator project (containing `run.sh`, `input/`, `output/`). op-tester writes NumPy-generated tensors into `input/*.bin`, executes the `command`, then reads the configured output files before comparing against reference results. Specify `--backend npu --chip ascend910b` (or whichever SOC you target) to activate this backend.

Use `op-tester run --help` for the full option list.

## Extending Operators & Backends
- Register operator descriptors or swap defaults via plugin modules imported before CLI execution (`OP_TESTER_PLUGINS=my_pkg.plugins`).
- Implement custom backends by subclassing `BackendDriver` and registering via `backend_manager.register(...)`. See `docs/op_tester_architecture.md#3-extending-op-tester-with-custom-gpunpu-backends` for detailed guidance, including the Ascend command backend.
...
- Ship company-specific backends/generators as companion packages that depend on `op-tester` and auto-register plugins on import.
...
- `docs/op_tester_design_plan.md` – project goals, phased plan, and risks.
- `docs/op_tester_architecture.md` – mermaid architecture diagram, core components, extension instructions, packaging guidance.
